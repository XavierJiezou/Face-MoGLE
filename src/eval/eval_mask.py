...
class VitExtractor:
    ...
    def __init__(self, model_name, device):
        # pdb.set_trace()
        self.model = torch.hub.load('facebookresearch/dino:main', model_name, model_dir="checkpoints").to(device)
        self.model.eval()
...
class PairedImageDataset(Dataset):
    def __init__(self, real_dir, fake_dir):
        self.real_dir = natsorted(glob(os.path.join(real_dir,"*.jpg")))
        
        self.fake_dir = natsorted(glob(os.path.join(fake_dir,"*.jpg")))

    def __len__(self):
        return len(self.real_dir)

    def __getitem__(self, idx):
        real_img = np.array(Image.open(self.real_dir[idx])) / 255.
        fake_img = np.array(Image.open(self.fake_dir[idx])) / 255.

        # (H, W, C) -> (C, H, W)
        real_tensor = torch.from_numpy(real_img).permute(2, 0, 1).float()
        fake_tensor = torch.from_numpy(fake_img).permute(2, 0, 1).float()
        return fake_tensor, real_tensor

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--real_dir",default="visulization/mmcelebahq/face")
    parser.add_argument("--fake_img",default="visulization/face-mogle/512/face")
    parser.add_argument("--output_dir",default="eval_result")
    args = parser.parse_args()
    fake_dir = args.fake_img
    real_dir = args.real_dir
    name = fake_dir.split(os.path.sep)[-3]
    dataset = PairedImageDataset(real_dir, fake_dir)
    dataloader = DataLoader(dataset, batch_size=64, shuffle=False, num_workers=4)

    model = DinoStructureLoss()
    ...
    with torch.no_grad():
        for fake_batch, real_batch in tqdm(dataloader):
            ...
            loss = model.calculate_global_ssim_loss(fake_batch, real_batch)
            dino_structure_dist.append(loss.item())

    mean_loss = sum(dino_structure_dist) / len(dino_structure_dist)
